...
const navItems = [
  ...
  { icon: MessageSquare, label: "Messages", path: "/inbox",     guestGated: true },
  { icon: User,          label: "Profile",  path: "/inventory", guestGated: true },
];
  ...
  useEffect(() => {
    const compute = () => {
      if (!isDemoUser(sessionRef.current?.user?.email)) {
        try {
          setUnreadCount(localStorage.getItem("inbox_unread_real") ? 1 : 0);
        } catch {
          setUnreadCount(0);
        }
        return;
    ...
    window.addEventListener("uniques:new-message", compute);
    return () => {
      window.removeEventListener("focus", compute);
      document.removeEventListener("visibilitychange", compute);
      window.removeEventListener("uniques:new-message", compute);
    };
  }, []);
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                key={item.path}
                onClick={() => {
                  // Gate protected tabs for guests
                  if (isGuest && item.guestGated) {
                    if (item.path === "/inventory") {
                      // Profile tab → send directly to login (not modal)
                      router.push("/login");
                      return;
                    }
                    const ctx = item.path === "/inbox" ? "access your messages" : "access your profile";
                    setGuestContext(ctx);
                    return;
                  }
